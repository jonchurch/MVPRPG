...
class Messenger {
  ...
  handleCallback(msg){
    const message = new Message(Message.mapCallbackMessage(msg))
    const callbackData = message.data
    console.log('CALLBACK MSG:\n',msg);

    console.log('callbackData:\n',callbackData);
    if (callbackData === 'LOOK') {
      return handlers.look.getLookString(message, this.bot)
    }
    if (callbackData === 'EXIT/EXPAND') {
      return handlers.commands.updateView(message, this.bot)
    }

      }

  handleText(msg) {
    const message = new Message(Message.mapTextMessage(msg))
